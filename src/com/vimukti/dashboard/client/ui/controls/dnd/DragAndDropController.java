	...
	private DragHandler getDragHandler() {
		return new DragHandler() {
			...
			public void onPreviewDragEnd(DragEndEvent event)
				...
				if (!((IDroppable) dropTarget).canDroppable((IDraggable) event
						.getContext().draggable)) {
			...
			public void onDragEnd(DragEndEvent event) {
				...
				if (dropTarget == null || context.draggable == null) {
					logger.warning("drop target not founded, so we are not dropping dragged widget");
					return;
				}

				dropTarget.onDrop((IDraggable) context.draggable);
